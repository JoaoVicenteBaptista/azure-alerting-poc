# Azure Alerting POC — Implementation Plan

> **REQUIRED SUB-SKILL:** Use the executing-plans skill to implement this plan task-by-task.

**Goal:** Build a Terraform-based POC that provisions Azure infrastructure (Function App + Service Bus + Key Vault + App Insights) with 9 alerts routed to Teams, plus a minimal .NET 10 isolated function that accepts POST requests and drops messages into a Service Bus queue.

**Architecture:** Single Terraform root module split across context files (providers, variables, key_vault, function_app, service_bus, alerts, outputs). Remote state in Azure Storage with locking. Teams webhook secret stored in Key Vault, referenced by Action Group. .NET 10 isolated function uses `ServiceBusClient` via DI with structured logging and correlation IDs.
